/*
   ...
   $Id: socket_sys_win.cpp,v 1.2 2006/11/07 14:22:43 gian Exp $
...
{
   m_lastError = 0;
   int count = s_select( (SOCKET) m_systemData, msec, 0 );
...
{
   m_lastError = 0;
   int count = s_select( (SOCKET) m_systemData, msec, 1 );
   if ( count == SOCKET_ERROR ) {
      m_lastError = WSAGetLastError();
      return false;
   }
...
bool Socket::bind( Address &addr, bool packet, bool broadcast )
{
   ...
   if ( m_systemData != 0 ) {
      closesocket( (SOCKET) m_systemData );
      m_systemData = 0;
   }
...
int32 TCPSocket::recv( byte *buffer, int32 size )
{
   ...
   int read = ::recv( (SOCKET) m_systemData, (char *)buffer, size, 0 );
   if ( read < 0 ) {
      m_lastError = WSAGetLastError();
      return -1;
   }
...
int32 TCPSocket::send( byte *buffer, int32 size )
{
   ...
   int sent = ::send( (SOCKET) m_systemData, (char *) buffer, size, 0 );
   if ( sent < 0 ) {
      m_lastError = WSAGetLastError();
      return -1;
   }
...
{
   if ( shutdown( (SOCKET ) m_systemData, SD_RECEIVE ) == SOCKET_ERROR )
   {
      m_lastError = WSAGetLastError();
      return false;
   }
...
{
   if ( shutdown( (SOCKET ) m_systemData, SD_SEND ) == SOCKET_ERROR ) {
      m_lastError = WSAGetLastError();
      return false;
   }
   // no wait
...
{
   if ( shutdown( (SOCKET ) m_systemData, SD_BOTH ) == SOCKET_ERROR )
   {
      m_lastError = WSAGetLastError();
      return false;
   }
...
bool TCPSocket::isConnected()
{
   ...
   int status = s_select_connect( (SOCKET) m_systemData, m_timeout );
   if ( status == 1 ) {
      m_connected = true;
      return true;
   }
...
{
   SOCKET srv = (SOCKET) m_systemData;
...
{
   int iOpt = mode ? 1:0;
   SOCKET skt = (SOCKET) m_systemData;
   setsockopt( skt, SOL_SOCKET, SO_BROADCAST, (const char *) &iOpt, sizeof( iOpt ));
}
