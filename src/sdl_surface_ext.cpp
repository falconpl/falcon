...
{
   CoreObject *self = vm->self().asObject();
   SDLSurfaceCarrier *carrier = static_cast<SDLSurfaceCarrier*>( self->getUserData() );
   SDL_LockSurface( carrier->m_surface );
   carrier->m_lockCount++;
}
...
FALCON_FUNC SDLSurface_UnlockSurface( ::Falcon::VMachine *vm )
{
   ...
   SDLSurfaceCarrier *carrier = static_cast<SDLSurfaceCarrier*>( self->getUserData() );
   carrier->m_lockCount--;
   SDL_UnlockSurface( carrier->m_surface );
}
...
FALCON_FUNC SDLSurface_LockIfNeeded( ::Falcon::VMachine *vm )
{
   CoreObject *self = vm->self().asObject();
   SDLSurfaceCarrier *carrier = static_cast<SDLSurfaceCarrier*>( self->getUserData() );
   if( SDL_MUSTLOCK( carrier->m_surface ) )
   {
      SDL_LockSurface( carrier->m_surface );
      carrier->m_lockCount++;
   }
