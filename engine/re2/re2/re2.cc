...
bool RE2::Replace(string *str,
  ...
  fassert(vec[0].begin() >= str->data());
  fassert(vec[0].end() <= str->data()+str->size());
  str->replace(vec[0].data() - str->data(), vec[0].size(), s);
  return true;
}

bool RE2::Replace(Falcon::String&  str,
                 const RE2& re,
                 const StringPiece& rewrite)
{
   Falcon::AutoCString cstr( str );
   std::string temp( cstr.c_str(), cstr.length() );
   bool status = RE2::Replace( &temp, re, rewrite );
   if( status )
   {
      str.fromUTF8( temp.c_str() );
      return true;
   }
...
int RE2::GlobalReplace(Falcon::String& str,
                 const RE2& re,
                 const StringPiece& rewrite)
{
   Falcon::AutoCString cstr( str );
   std::string temp( cstr.c_str(), cstr.length() );
   int status = RE2::GlobalReplace( &temp, re, rewrite );
   if( status > 0 )
   {
      str.fromUTF8( temp.c_str() );
   }
...
{
  string temp;
  bool result = Extract(text, re, rewrite, &temp);
  if( result )
  {
     out.fromUTF8(temp.c_str());
     return true;
  }
  return false;
}
...
bool RE2::Arg::parse_null(const char*, int, void* dest) {
