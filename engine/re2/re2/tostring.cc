...
class ToStringWalker : public Regexp::Walker<int> {
 public:
  ...
  virtual int ShortVisit(Regexp* , int ) {
    return 0;
  }
...
void Regexp::ToFalconString(Falcon::String& tgt ) {
  string t;
  ToStringWalker w(&t);
  w.WalkExponential(this, PrecToplevel, 100000);
  if (w.stopped_early())
...
int ToStringWalker::PreVisit(Regexp* re, int parent_arg, bool* ) {
  int prec = parent_arg;
  int nprec = PrecAtom;
...
int ToStringWalker::PostVisit(Regexp* re, int parent_arg, int ,
                              int* , int ) {
  int prec = parent_arg;
  switch (re->op()) {
    case kRegexpNoMatch:
