...
const string& RegexpStatus::CodeText(enum RegexpStatusCode code) {
  if (code < 0 || static_cast<size_t>(code) >= arraysize(kErrorStrings))
...
class NumCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored PreVisit(Regexp* re, Ignored ignored, bool*) {
    if (re->op() == kRegexpCapture)
      ncapture_++;
    return ignored;
  }
  virtual Ignored ShortVisit(Regexp*, Ignored ignored) {
...
class NamedCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  Ignored PreVisit(Regexp* re, Ignored ignored, bool*) {
