...
/*#
   @method comp Dict
   @brief Adds elements to this array through a filter.
   ...
   This method adds a pair [key, value] one item at a time to this array.

   If @b item is a range,
   (must not be open), all the values generated by the range must be passed throug
   a @b filter that will generate a [key, value] pair using that range.

   If @b item is a sequence (array,
   list, or any other object providing the sequence interface), all the values
   in the item will be appended to this dictionary; each entry of the sequence
   must be a pair, or it must be passed by a @b filter returning a pair.

   If @b item is a callable item, it is called repeatedly to generate the sequence.
   All the pairs it returns will be appended, until
   ...
   are first passed to to it. In this case, the @b filter function must return
   a pair, but it can receive any element.
*/
