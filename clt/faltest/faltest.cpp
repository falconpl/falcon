/*
   ...
   FILE: faltest.cpp
   $Id: faltest.cpp,v 1.27 2007/08/11 19:02:32 jonnymind Exp $
         ...
         String &reason, String &trace )
{
   ...
   // we need a place where to store the bytecode generated by the compiler.
   Stream *bytecode_stream;
   ...
   if ( opt_compasm )
   {
      ...
      if( opt_compmem )
      {
         bytecode_stream = new StringStream;
      }
      else {
         // create a temporary file
         FileStream *fsbin = new FileStream;
         fsbin->create( "temp_bytecode", FileStream::e_aUserWrite | FileStream::e_aReadOnly );
         bytecode_stream = fsbin;
         if ( ! bytecode_stream->good() )
         {
            reason = "Failed to open temporary binary stream temp_bytecode";
            delete asm_stream;
      ...
      AsmCompiler compiler( scriptModule, asm_stream, bytecode_stream );
      ...
      if( opt_compmem ) {
         ...
         if ( ! bytecode_stream->good() )
         {
            reason = "Failed to open temporary bytecode stream temp_bytecode";
            delete bytecode_stream;
            return false;
         }
      }
