...
#undef SRC
#define SRC "modules/native/feathers/timestamp/timestamp_ext.cpp"

/*#
   @beginmodule feathers.timestamp
*/
...
{
  TimeStamp* self = static_cast<TimeStamp*>(instance);
  value.setInteger(self->date().toMilliseconds());
}
...
static void set_msSinceEpoch( const Class*, const String&, void *instance, const Item& value )
{
  ...
  self->date().fromMilliseconds( value.forceInteger() );
}
...
ClassTimeStamp::ClassTimeStamp():
         Class("TimeStamp")
{
   setConstuctor( new _classTimeStamp::Function__constructor );
...
*/
FALCON_DECLARE_FUNCTION(getDisplacement, "tz:N")
void  Function_getDisplacement::invoke ( ::Falcon::VMContext* ctx, int32 )
{
...
FALCON_DECLARE_FUNCTION(getName, "tz:N")
void  Function_getName::invoke ( ::Falcon::VMContext* ctx, int32 )
 {
   Item *i_tz = ctx->param(0);
   if( i_tz == 0 || ! i_tz->isOrdinal() )
   {
   ...
   if ( tz < 0 || tz >= 32 )
   {
    ...
    ctx->returnFrame( FALCON_GC_HANDLE(new String(  TimeStamp::getRFC2822_ZoneName( (TimeStamp::TimeZone) tz ) ) ) );
 }
...
/*#
    ...
    @endcode
*/
FALCON_DECLARE_FUNCTION(getLocal, "tz:N")
void  Function_getLocal::invoke ( ::Falcon::VMContext* ctx, int32 )
{
   ctx->returnFrame( (int64) TimeStamp::getLocalTimeZone() );
}
}
         ...
         Class("TimeZone")
{
   ...
   addMethod( new _classTZ::Function_getName, true );
...
void* ClassTimeZone::createInstance() const
{
   // pure virtual class
   return 0;
}

void ClassTimeZone::dispose( void* ) const
{
   // pure virtual class
}

void* ClassTimeZone::clone( void* ) const
{
   // pure virtual class
   return 0;
}
...
{
   static Class* tsc = module()->getClass("TimeStamp");
   ...
   ctx->returnFrame( FALCON_GC_STORE(tsc, ts1) );
}
...
void Function_currentTime::invoke ( VMContext* ctx, int32 )
{
   ...
   TimeStamp *ts = new TimeStamp;
   ts->currentTime();
   ctx->returnFrame( FALCON_GC_STORE(tsc, ts) );
}

}
} // namespace Falcon::Ext

/* end of timestamp_ext.cpp */
