...
/*#
   @main The Multithreading Support Module.
   ...
   @note In version 0.8.10, the module loader keeps an extra
   reference to this module, which must exist untill the linking
   VM is definitely closed. Embedding applications should pre-cache
   this module and decref it after all the VM using it are destroyed,
   or face a relatively small memory leak each time the module is linked.
   This problem will be resolved in 0.8.12.
*/
...
/*#
   ...
   The @a Thread.wait method conforms to the VM Interruption protocol,
   ...
   can be generated by the @a Thread.stop method, that will send an asynchronous
