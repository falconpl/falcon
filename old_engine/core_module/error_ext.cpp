...
/*#
   ...
   @prop boxed Boxed error. Similar to subError, but this is a single error that
               was separately raised, and caused an higher-priority error to be
               raised next. For example, included script may raise an error that
               generates then a "failed to include" error at top-level. The inner
               error generated by the included script is available as the boxed
               error.
*/
...
/*#
   ...
   successful both on the stream side (correct load) and on the code side
...
/*#
   ...
   or some illegal instruction parameter. Scripts can hardly receive it, if not as a notification
...
/*#
   ...
   semantic is appropriate (i.e. when providing them nonexisting property
   names, or when requesting to access an nonexisting range in an array).
*/
