...
#pragma message ("Using modified VC6 XTREE for Falcon...")
        ...
        public:
                ...
                void _Dec()
                        {if (_Color(_Ptr) == _Red
                                ...
                                _Ptr = _Right(_Ptr);
                        /* JC else if (_Left(_Ptr) != _Nil) */
                        else if ( _IsNotNil(_Left(_Ptr)) )
                                ...
                                _Ptr = _P; }}
                void _Inc()
                        {/* JC if (_Right(_Ptr) != _Nil)*/
                         if ( _IsNotNil(_Right(_Ptr)) ) 
...
protected:
        ...
        _Nodeptr _Copy(_Nodeptr _X, _Nodeptr _P)
                {_Nodeptr _R = _X;
                /* JC1 for (; _X != _Nil; _X = _Left(_X)) */
                for (; _IsNotNil(_X); _X = _Left(_X))
        ...
        void _Erase(_Nodeptr _X)
                {/* JC for (_Nodeptr _Y = _X; _Y != _Nil; _X = _Y) */
                for (_Nodeptr _Y = _X; _IsNotNil(_Y); _X = _Y)
                        {_Erase(_Right(_Y));
                        _Y = _Left(_Y);
                        _Destval(&_Value(_X));
                        _Freenode(_X); }}
        ...
        iterator _Insert(_Nodeptr _X, _Nodeptr _Y, const _Ty& _V)
                ...
                ++_Size;
                /* JC if (_Y == _Head || _X != _Nil */
               if (_Y == _Head || _IsNotNil(_X)
        ...
        static _Nodeptr _Max(_Nodeptr _P)
                {/* JC while (_Right(_P) != _Nil) */
                  while ( _IsNotNil(_Right(_P)) )
                        _P = _Right(_P);
                return (_P); }
        static _Nodeptr _Min(_Nodeptr _P)
                {/* JC while (_Left(_P) != _Nil) */
                  while ( _IsNotNil(_Left(_P)) )
                        _P = _Left(_P);
                return (_P); }
        _Nodeptr& _Rmost()
                {return (_Right(_Head)); }
        ...
        void _Rrotate(_Nodeptr _X)
                ...
                _Left(_X) = _Right(_Y);
                /* JC1 if (_Right(_Y) != _Nil) */
                  if ( _IsNotNil( _Right(_Y) ))
...
/*
 * File modified to make it work across DLL calls by Giancarlo Niccolai
 * Parts changed are marked by JC comment.
 *
 * Copyright (C) 2004 Giancarlo Niccolai
 *
 * The changes are distributed under the same statement as above,
 * including, but not limited to, the exclusion of any express or implied
 * warranty 
 */