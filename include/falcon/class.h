   ...
   */
   ...
   /** Summon an instance.
    ...
    \note The default behavior is that of returning or setting a property value if the summoned
          ...
          name.
    */
   virtual void op_summon( VMContext* ctx, void* instance, const String& message, int32 pCount ) const;

   /** Checks if an instance resoponds to a summon message.
    \param ctx the virtual machine context that will receive the result.
    \param instance the summoned instance (or 0 on flat items)
    \param message pCount number of parameters for the summoning

    \b signature: (0: instance) --> (1: instance) (0: true or false)

    This method call is generated by the virtual machine when a summon expression wants to
    check for the method availability.

    \note The default behavior is to set availability to true if the entity has the
          required property or method.
    */
   virtual void op_respondTo( VMContext* ctx, void* instance, const String& message ) const;
