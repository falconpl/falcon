...
bool URI::internal_parseQuery( const String &src, uint32 pEnd, bool parseQuery, bool bDecode )
{
   ...
   while ( pEnd < len )
   {
      ...
      else if ( chr  == '&' )
      {
         ...
         // save this key
         m_queryMap->insert(std::make_pair( tempKey, val) );
         bIsValue = false;
         pStart = pEnd + 1;
      }
      else if ( chr == '#' )
...
bool URI::hasField( const String &f ) const
{
   ...
   QueryMap::iterator res =  m_queryMap->find( f );
   return res != m_queryMap->end();
}
...
bool URI::getField( const String &key, String &value ) const
{
   ...
   QueryMap::iterator res =  m_queryMap->find( key );
   if ( res != m_queryMap->end() )
   {
      value = (*res).second;
      return true;
   }
...
void URI::setField( const String &key, const String &value )
{
   ...
   m_queryMap->insert( std::make_pair(key, value) );
}
...
const String &URI::makeQuery() const
{
   ...
   {
      QueryMap::iterator iter = m_queryMap->begin();
      m_query += URLEncode( (*iter).first ) + "=" +
                  URLEncode( (*iter).second );
      QueryMap::iterator end = m_queryMap->end();
      while( iter != end )
      {
         iter++;
         m_query += "&";
