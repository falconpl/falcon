...
public:
   ...
   */
   ...
   /** Prepares iteration.
    \param vm the virtual machine that will receive the result.
    \param instance the instance (or 0 on flat items)
    \note The operand is ternay
    
    \b signature: (2: seq) (1: iter ) (0: item|break)
    ...
    Three items are left in the stack by the caller; op_iter should fill
    the VMContext::opcodeParam(1) with an iterator that can then be used
    by op_next, or stored elsewhere. The item at top of the stack, 
    opcodeParam(0), is reserved for op_next.

    In case the class can't create an iterator, it should just leave nil
    at opcodeParam(1).    
    */
